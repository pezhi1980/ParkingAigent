...
## 9. تصمیم استراتژیک — 2026-04-04

### مسیر آینده: فقط ARCore/ARKit (SDK Agent)

تصمیم گرفته شد که توسعه حرفه‌ای فقط بر روش ARCore/ARKit (SDK Agent) تمرکز کند.
روش Gyroscope در اپ فعلی به عنوان نمونه اولیه باقی می‌ماند و تغییر نمی‌کند.

**دلیل:**
- خطای ARCore/ARKit: ≤ 0.50m (مستند در `ar_measurement_strategy.md`)
- خطای Gyroscope: 3–6m (غیر قابل دفاع قانونی)

---

### درباره عدد 14m در اپ فعلی

عدد `14m` در `MeasureCamera.jsx` **عمدی و صحیح** است — نه اشتباه.

**منطق:**

```
قانون دانمارک (بالاترین آستانه) = 10m (تقاطع)
خطای gyroscope                  = ±4–6m
─────────────────────────────────────────
Safety buffer لازم              = 10m + 4m = 14m
```

یعنی اگر اپ gyro-based بگوید ۱۴ متر فاصله داری، احتمال خیلی بالایی وجود دارد که
فاصله واقعی حداقل ۱۰ متر باشد (مطابق قانون). این buffer عمداً برای جبران خطای اندازه‌گیری است.

**نتیجه: `14m` در اپ فعلی تغییر نمی‌کند.**
...
| موضوع | تصمیم |
|---|---|
| روش اندازه‌گیری آینده | **ARCore/ARKit فقط** |
| اپ فعلی (gyro) | نگه داشته می‌شود، تغییر نمی‌کند |
| `LEGAL_LIMIT = 14.0` در اپ فعلی | **عمدی است** — safety buffer برای جبران خطای gyro |
| SDK Agent thresholds (5m/10m/12m) | قفل‌شده و صحیح — قانون دانمارک |
| Decision state vocabulary | در SDK Agent قفل است، اپ فعلی مستقل باقی می‌ماند |

---

## 10. تغییرات

هیچ تغییری در هیچ‌کدام از پروژه‌ها ایجاد نشد. این سند فقط گزارش تحلیل و تصمیم‌گیری است.
